...
export default function Enfoques() {
  const lenf = textos.gestionintegrada.lenfoques;
  return (
      ...
      <div className="w-full h-full flex ">
          ...
          {lenf.map((text, id) => {
            return (
              ...
              >
                <div
                  key={id}
                  className={`w-full h-full lg:h-2/3 border-2 border-white bg-${textos.gestionintegrada.cenfoques[id]} flex flex-col items-center justify-center`}
                >
                  <h1
                ...
                <div className="border-r-2 hidden lg:block text-sm  border-slate-700">
                  <p>{textos.gestionintegrada.enfoques[id]}</p>
                </div>
              </motion.div>
            );
          })}
        ...
        <div className="w-1/2 lg:hidden h-full gap-8 flex flex-col ">
          {lenf.map((text, id) => {
            return (
              <p className="" key={id}>
                {textos.gestionintegrada.enfoques[id]}
              </p>
            );
          })}
        </div>
